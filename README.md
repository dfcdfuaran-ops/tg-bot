...
- 💳 Интеграция с платежными системами (Yookassa, Cryptomus, Telegram Stars и др.)
...
```bash
# Остановить бота
cd /opt/tg-sell-bot && docker compose down

# Запустить бота
cd /opt/tg-sell-bot && docker compose up -d

# Просмотр логов
cd /opt/tg-sell-bot && docker compose logs -f

# Перезагрузка
cd /opt/tg-sell-bot && docker compose restart

# Полное удаление
cd /opt/tg-sell-bot && docker compose down -v
```
