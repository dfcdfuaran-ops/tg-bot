...
async def sync_completed_getter(
    ...
    **kwargs: Any,
    ...
    completed: dict = dialog_manager.dialog_data["completed"]
    # Определяем направление синхронизации по наличию ключей результата
    # sync_all_users_from_panel_task возвращает: total_panel_users, total_bot_users, added_users, added_subscription...
    # sync_bot_to_panel_task возвращает: total_bot_users, created, updated, skipped, errors
    
    if "total_panel_users" in completed:
        # Синхронизация из панели в бот
        completed["message_key"] = "msg-importer-sync-completed"
    else:
        # Синхронизация из бота в панель
        completed["message_key"] = "msg-importer-sync-bot-to-panel-completed"
