...
class Promocode(BaseSql, TimestampMixin):
    ...
    allowed_plan_ids: Mapped[list[int]] = mapped_column(JSON, nullable=True, default=[])
...
class PromocodeActivation(BaseSql):
    ...
    # Сохраняем предыдущую скидку пользователя для восстановления при удалении промокода
    previous_discount: Mapped[int] = mapped_column(
        Integer,
        nullable=False,
        default=0,
    )
    # Сохраняем срок действия предыдущей скидки (None = постоянная скидка)
    previous_discount_expires_at: Mapped[Optional[datetime]] = mapped_column(
