...
show_full_menu() {
    set +e  # Отключаем exit on error для функции меню
    local selected=0
    local options=("�  Просмотр логов" "⚙️   Изменить настройки" "🔄  Обновить" "🔃  Перезагрузить бота" "⬇️   Выключить бота" "⬆️   Включить бота" "🔄  Переустановить" "🧹  Очистить данные" "🗑️   Удалить бота" "❌  Выход")
    local num_options=${#options[@]}
