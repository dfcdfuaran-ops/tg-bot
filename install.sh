...
# 3. Создание и очистка папок для данных
(
  # Очистить старые assets если они есть (для первого запуска нужны дефолтные)
...
(
  ...
  # Удалить скрипты (они уже встроены в Docker образ)
