...
show_full_menu() {
    ...
    if [ $UPDATE_AVAILABLE -eq 1 ]; then
        update_status=" ${YELLOW}( Доступно обновление! )${NC}"
    fi
    
    local options=("ℹ️   Просмотр логов" "⚙️   Изменить настройки" "🔄  Обновить${update_status}" "🔃  Перезагрузить бота" "⬇️   Выключить бота" "⬆️   Включить бота" "🔄  Переустановить" "🧹  Очистить данные" "🗑️   Удалить бота" "❌  Выход")
    local num_options=${#options[@]}
